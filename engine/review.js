 ...
 * no es el que trae el transcript (`engine/reloj.js`). Se arma al servir y no se
 * guarda porque cuesta milisegundos y porque el Backup tiene que seguir diciendo
 * lo que midió Whisper: de ahí sale todo lo demás.
 */
...
function loadReview(params) {
    ...
    // se usa esa, que es la buena y es también la que decidió los cortes. Si no,
    // se corrige contra la onda con `retimeo`, que sigue andando sin pedirle al
    // editor que vuelva a transcribir nada.
    const palabras = transcript ? transcript.words || [] : [];
    const conDtw = reloj.traeDtw(palabras);
    ...
    return {
        ...
        words: armado ? armado.palabras : palabras,
        ...
        reloj: armado
            ? { como: conDtw ? 'dtw' : 'onda', ...armado.stats }
            : { como: 'whisper' },
